			...
			<dl class="icon" style="background-image: url(<?php echo $_forumrow_val['FORUM_FOLDER_IMG_SRC']; ?>); background-repeat: no-repeat;">
				...
				<?php } else if (! $_forumrow_val['S_IS_LINK']) {  ?>
					...
					<dd class="lastpost"><span>
						<?php if ($_forumrow_val['U_UNAPPROVED_TOPICS']) {  ?><a href="<?php echo $_forumrow_val['U_UNAPPROVED_TOPICS']; ?>"><?php echo (isset($this->_rootref['UNAPPROVED_IMG'])) ? $this->_rootref['UNAPPROVED_IMG'] : ''; ?></a><?php } if ($_forumrow_val['LAST_POST_TIME']) {  ?><dfn><?php echo ((isset($this->_rootref['L_LAST_POST'])) ? $this->_rootref['L_LAST_POST'] : ((isset($user->lang['LAST_POST'])) ? $user->lang['LAST_POST'] : '{ LAST_POST }')); ?></dfn> <?php if ($_forumrow_val['ALTT_LINK_NAME_SHORT']) {  ?><a <?php if ($_forumrow_val['ALTT_STYLE_BOLD'] || $_forumrow_val['ALTT_STYLE_ITALIC'] || $_forumrow_val['ALTT_STYLE_ADV']) {  ?>style="<?php if ($_forumrow_val['ALTT_STYLE_BOLD']) {  ?>font-weight: bold;<?php } if ($_forumrow_val['ALTT_STYLE_ITALIC']) {  ?>font-style: italic;<?php } if ($_forumrow_val['ALTT_STYLE_ADV']) {  echo $_forumrow_val['ALTT_STYLE_ADV2']; } ?>"<?php } ?> href="<?php echo $_forumrow_val['U_ALTT_LINK']; ?>" title="<?php echo $_forumrow_val['ALTT_LINK_NAME']; ?>"><?php echo $_forumrow_val['ALTT_LINK_NAME_SHORT']; ?></a><br /><?php } ?> <?php echo ((isset($this->_rootref['L_POST_BY_AUTHOR'])) ? $this->_rootref['L_POST_BY_AUTHOR'] : ((isset($user->lang['POST_BY_AUTHOR'])) ? $user->lang['POST_BY_AUTHOR'] : '{ POST_BY_AUTHOR }')); ?> <?php echo $_forumrow_val['LAST_POSTER_FULL']; ?>
