                ...
                <table id="maintable">
                    <tr>
                        ...
                        <td width="780" valign="top">
                            ...
                            <?php
                            //Количество новостей на страницу - 5 шт
                            $num = 5;
                            ...
                            //Запрос к базе данных - выбор кол-ва новостей в таблице
                            $result = mysql_query("SELECT COUNT(*) FROM articles WHERE article_type = 'n'");
                            //Получение результата запроса к БД
                            $posts = mysql_result($result, 0);
                            //Подсчет сколько всего страниц по 5 новстей существует
                            $total = intval(($posts - 1) / $num) + 1;
                            //Подсчет сколько новостей и с какой позиции отсчитывать для данной страницы
                            $page = intval($page);
                            if (empty($page) or $page < 0) $page = 1;
                            if ($page > $total) $page = $total;
                            $start = $page * $num - $num;

                            //Формирование запроса для выборки из БД новостей для данной страницы
                            ...
                            //Тогда выводим расширенную версию отображения новости
                            //C возможностю удаления и редакрирования
                            if (isset($_SESSION[name]) && ($_SESSION[priv] == 'a')) {

                                //Цикл для вывода новостей, цикл завершится, когда в результате не будет рядков
                                while ($row = mysql_fetch_array($result)) {
                                    ...
                                    //Вывод айди новости в невидимую форму, для кнопки редактирования новости
                                    ...
                                    //Вывод айди новости в невидимую форму, для кнопки удаления новости
                                    echo('<input type="hidden" name="news_id" value="' . $row[article_id] . '"/>');
                                    ...
                                    echo('<td colspan="4">');
                                    echo('<div id="content">');
                                    //Вывод краткого содержания новости
                                    ...
                                    echo('<tr>');
                                    echo('<td>');
                                    //Вывод имени автора новости
                                    echo("<div id='author'>" . $row['user_login'] . "</div>");
                                    echo('</td>');
                                    echo('<td>');
                                    //вывод даты написания новости
                                    echo("<div id='data'>" . $row['article_date'] . "</div>");
                                    echo('</td>');
                                    //Формирование запроса в БД для кол-ва коментариев к новости
                                    ...
                                    //Вывод кол-ва новостей в текст ссылки и формирование
                                    // ссылки на открытие полного текста новости с помощью ее айди
                            ...
                            //Выводим обычное отображение новости без доп. кнопок
                            else {
                                while ($row = mysql_fetch_array($result)) {
                                    echo("<table border='0' id='article'>");
                                    echo("<tr>");
